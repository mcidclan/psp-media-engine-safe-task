...
The answer came from the DMACPLUS hardware registers. Some time ago, I had the opportunity to experiment with them and understand how they work (thanks again to davee for the advice). Beyond the functions implemented by Sony, DMACPLUS allows transferring data in kernel mode to anywhere in the Media Engine's local EDRAM. The solution is fairly obvious from there: use DMACPLUS in word transfer mode to copy a single word, the new address of our custom function, directly into the Syscall table.
